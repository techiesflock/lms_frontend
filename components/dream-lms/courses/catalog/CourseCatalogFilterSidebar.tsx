...
const CATEGORIES = ["Development", "Design", "Business", "Marketing", "Data Science", "IT & Software"];
const INSTRUCTORS = ["Nicole Brown", "Ana Reyes", "Andrew Pirtle", "Christy Garner", "Justin Gregory"];
const LEVELS = ["Beginner", "Intermediate", "Advanced"];
const PRICES = ["All", "Free", "Paid"];

export function CourseCatalogFilterSidebar({ idPrefix, modern = false }: CourseCatalogFilterSidebarProps) {
  if (!modern) {
  ...
  return (
    <div className="mw-catalog-filters">
      ...
      <FilterGroup title="Level" defaultOpen>
        {LEVELS.map((label) => (
          <FilterCheck key={label} name={`${idPrefix}-level`} label={label} />
        ))}
      </FilterGroup>

      <FilterGroup title="Price">
        {PRICES.map((label) => (
          <FilterCheck key={label} name={`${idPrefix}-price`} label={label} type="radio" />
        ))}
      </FilterGroup>

      <FilterGroup title="Rating">
        {[4.5, 4.0, 3.5, 3.0].map((min) => (
      ...
      <FilterGroup title="Instructor">
        {INSTRUCTORS.map((label) => (
          <FilterCheck key={label} name={`${idPrefix}-instructor`} label={label} />
        ))}
      </FilterGroup>
    </div>
  );
